...
export default function Members() {
  ...
  async function handleSendReminders() {
    ...
    try {
      const res = await api.post('/members/recordatorios')
      ...
      const msg = err.response?.data?.error || 'Error al enviar recordatorios'
        ...
        <div style={{
          ...
          {reminderResult.type === 'success' ? (
            <>
              ...
              <span>
                ✅ Recordatorios enviados: <strong>{reminderResult.data.total}</strong> mensajes (
                {reminderResult.data.por_vencer > 0 && `${reminderResult.data.por_vencer} por vencer `}
                {reminderResult.data.vence_hoy > 0 && `${reminderResult.data.vence_hoy} vencen hoy `}
                {reminderResult.data.vencidos > 0 && `${reminderResult.data.vencidos} vencidos `}
                )
                {reminderResult.data.fallidos > 0 && (
                  <span style={{ color: 'var(--danger)' }}> — {reminderResult.data.fallidos} fallidos</span>
                )}
              </span>
            </>
          ) : (
